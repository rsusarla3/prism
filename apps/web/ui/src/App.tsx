...
type View='home'|'core'|'future'|'library'; type Theme='light'|'dark';
...
export default function App(){const[view,setView]=useState<View>(()=>(location.hash.slice(1)as View)||'home');const[theme,toggle]=useTheme();useEffect(()=>{const fn=()=>setView((location.hash.slice(1)as View)||'home');addEventListener('hashchange',fn);return()=>removeEventListener('hashchange',fn)},[]);const go=(v:View)=>{location.hash=v};return <><header className="topbar"><button className="brand" onClick={()=>go('home')}><span>◇</span> prism</button><nav aria-label="Products"><button className={view==='library'?'active':''} onClick={()=>go('library')}>Library</button><button className={view==='core'?'active':''} onClick={()=>go('core')}>Core</button><button className={view==='future'?'active':''} onClick={()=>go('future')}>Future</button></nav><button className="icon-button" onClick={toggle} aria-label={`Switch to ${theme==='light'?'dark':'light'} theme`}>{theme==='light'?'◐':'◑'}</button></header><main>{view==='home'?<Home go={go}/>:view==='library'?<Library/>:view==='core'?<Core/>:<Future/>}</main><footer><span>Prism is an educational prototype.</span><span>One concept. As many ways as it takes.</span></footer></>}
const RAYS=[
