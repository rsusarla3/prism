# Prism — Hackathon Product and Engineering Specification
...
**Primary purpose of this file:** Keep human teammates and coding agents aligned while building Prism in parallel.
...
3. Do not turn Prism into a generic chatbot. The product is a guided learning workflow.
4. The learner must attempt a homework problem before Prism reveals the final answer.
5. Prism asks the learner's goal before starting instruction.
6. The user may explicitly highlight the exact content they want Prism to interpret.
7. V1 personal-finance data is entered manually. Preserve a clean provider interface for future bank integrations.
...
Prism is an adaptive Chrome learning copilot that understands the material a learner selects, asks what they are trying to accomplish, and teaches the concept through interactive methods that change when the learner gets stuck.

## 1.4 Core thesis

Most digital education products deliver one fixed explanation and then measure whether a learner succeeded. Prism instead:
...
Prism does **not** permanently label a person as a visual, auditory, or other type of learner. It learns which interventions appear effective for this learner, concept, context, and moment.
...
- Prism may identify and explain the underlying concept immediately.
- Prism may provide hints immediately.
- Prism may generate a similar worked example immediately.
- Prism may check the learner's work immediately.
- Prism reveals the final answer to the original homework problem **only after the learner makes a meaningful attempt**.
- The learner can request the answer after attempting.
- Prism must still explain the answer; it should not output only a result.
...
Before teaching, Prism asks what the learner wants to do.
...
- A future Plaid or equivalent connection should be addable without rewriting learning features.
- All V1 financial examples are educational simulations, not personalized investment instructions.

## 2.4 Interaction policy

Prism uses native interactive lessons:
...
Prism may detect likely struggle or disengagement and offer another learning method.
...
Prism has two branded experiences built on one shared platform.
...
Learners retain agency and produce work before Prism reveals the original answer.

## 6.2 Diagnose before explaining

Prism should ask a small diagnostic question instead of immediately delivering a long explanation.
...
Prism explains why it recommends another method and allows the learner to decline.
...
2. User right-clicks.
3. User selects **Learn with Prism**.
4. Prism opens the side panel.
5. Prism shows the selected content in a share-preview card.
6. Prism asks:
   - Explain this
   - Help me solve it
   - Check my work
...
8. Prism asks one diagnostic question.
9. Prism begins the selected mode.
10. Prism records attempts and interventions.
11. Prism offers a mode switch if indicated.
12. Prism verifies understanding with a new question.
13. Prism provides a summary and next step.
...
4. Prism asks the goal.
5. Prism asks what the learner already knows or gives a two-question diagnostic.
6. Prism maps the request to a known curriculum concept.
7. Prism starts the recommended mode.
8. If no approved curriculum object exists, Prism enters **Exploratory Mode** and clearly labels the content as AI-generated rather than curriculum-verified.
...
5. Prism detects the first issue or confirms the step.
6. Prism gives the least revealing useful hint.
7. Learner retries.
8. Hint strength increases only as needed.
9. After at least one meaningful attempt, **Show full solution** becomes available.
10. If selected, Prism reveals a step-by-step solution and identifies where it differs from the learner's attempt.
11. Prism gives a similar problem.
12. Mastery is updated based primarily on the similar problem, not on viewing the solution.

## 8.4 Check-my-work flow

1. User submits work.
2. Prism parses steps.
3. Deterministic verifier checks mathematical equivalence where possible.
4. Prism marks:
   - Correct steps
   - First incorrect step
   - Likely misconception
5. Prism asks the learner to correct the step.
6. Prism does not rewrite the entire assignment unless explicitly requested after an attempt.
...
3. User enters:
   ...
   - Annual fee
4. Prism displays assumptions prominently.
5. Graph updates with controls.
6. Prism asks prediction questions before revealing comparisons.
7. Prism explains:
...
9. Prism quizzes the user using a changed scenario.
...
- Prism provides education and simulations, not individualized investment recommendations.
...
Content script <----> Side panel
      |                  |
Page selection       Prism session UI
...
- Passwords
- Assignment submissions unrelated to Prism sessions
...
The web app is the persistent home for Prism.
...
```text
You are Prism, an adaptive learning coach.
...
- Mark unverified exploratory explanations.
...
class ManualFinancialDataProvider implements FinancialDataProvider {
  // Reads only user-entered Prism data.
}
```

A future Plaid adapter should implement the same interface in the web backend. It must not run inside the extension and must never expose provider access tokens to the browser.

---

# 24. UI and Design System

## 24.1 Shared Prism identity

The Prism metaphor means one concept can be refracted into multiple understandable representations.
...
- Prism asks the learner to repair it.
...
Prism reads learning context only after an explicit user action.
...
> Prism does not read this page automatically. Paste or type the non-sensitive concept you want help with.
...
> Prism cannot read this browser-protected page. Highlight and copy the material into the side panel instead.

## 28.2 Unknown concept

> Prism does not yet have a verified lesson for this concept. It can provide an AI-generated exploratory explanation, clearly labeled as unverified.
...
- [ ] Toolbar action opens Prism side panel.
...
> Most learning tools give everyone the same explanation. Prism keeps changing the method—while preserving the concept—until it clicks.

## School demo

1. Open an algebra problem in the browser.
2. Highlight the equation.
3. Right-click **Learn with Prism**.
...
> Prism is not another answer bot or another fixed course. It is a learning layer for the web: one concept, as many ways as it takes to make it click.

---

# 37. Roadmap Beyond the Hackathon
...
2. LLM keys and future financial tokens remain server-side.
...
7. Manual finance and future connected finance share an interface.
...
```text
You are contributing to the Prism hackathon repository.

Read `PRISM_HACKATHON_BUILD_SPEC.md` and `AGENTS.md` before making changes.

Prism is a Chrome learning copilot with the tagline:
“One concept. As many ways as it takes to make it click.”
...
- Plaid documentation: Link, Transactions, and Investments for future integration
...
A feature belongs in Prism when it helps complete this loop:

```text
Encounter confusing material
        ↓
Choose exactly what Prism may read
        ↓
